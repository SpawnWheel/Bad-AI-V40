...
class LiveTelemetryReader:
                ...
                if name in self.last_positions:
                    ...
                    if pos < old_pos and old_pos != 0:
                        ...
                        if overtaken:
                            ...
                            self._push_event("OVERTAKE", f"{name} overtook {overtaken_str} for P{pos} in {class_name}.", position=pos)
        ...
        for p in active_participants:
            ...
            if self.data.mRaceState == 2 and speed < 2.0 and dist > 10 and not accident_cooldown:
                if name not in self.accidents:
                    ...
                    self._push_event("ACCIDENT", f"Alert: {name} (P{pos} in {class_name}) is slow/stopped on track! Potential accident.", position=pos)
    ...
    def get_race_context(self) -> Dict[str, Any]:
        ...
        grid.sort(key=lambda x: x[0])
        standings = " | ".join([f"P{pos}: {name}" for pos, name in grid])
